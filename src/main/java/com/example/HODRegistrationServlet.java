...
@WebServlet("/HODRegistrationServlet")
@MultipartConfig(maxFileSize = 16177215)  // ~16MB for image upload
public class HODRegistrationServlet extends HttpServlet {
    @Override
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        // SQL query to insert data into the `hod` table
        String sql = "INSERT INTO hod (name, phone, email, Image, department_id) VALUES (?, ?, ?, ?, ?)";

        try {
            // Obtain database connection
            con = DBConnection.getConnection();
            ps = con.prepareStatement(sql);
            ps.setString(1, hodName);
            ...
            if (rowsInserted > 0) {
                ...
                response.getWriter().println("<h3>Error: HOD registration failed.</h3>");
            }
