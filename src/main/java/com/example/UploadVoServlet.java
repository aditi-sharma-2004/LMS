...
@WebServlet("/UploadVoServlet")
@MultipartConfig(maxFileSize = 16177215) // 16MB max file size
public class UploadVoServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        // Generate unique VO ID
        String voId = generateVoId();

        try {
            Class.forName("com.mysql.cj.jdbc.Driver");
            conn = DBConnection.getConnection();

            String sql = "INSERT INTO vo (vo_id, name, email, phone, Image, department_id) VALUES (?, ?, ?, ?, ?, ?)";
            ...
            if (row == 0) {
                response.getWriter().println("Error: Could not insert Verification Officer record.");
                return;
            }
            ...
            // Store Verification Officer login credentials
            String insertLoginSQL = "INSERT INTO vologin (vo_id, password) VALUES (?, ?)";
            int row2;
            try (PreparedStatement pstmt2 = conn.prepareStatement(insertLoginSQL)) {
                pstmt2.setString(1, voId);
            ...
            String subject = "Your Verification Officer Account Details";
            String body = "Dear " + name + ",\n\n" +
                        "Your Verification Officer account has been created successfully.\n" +
                        "VO ID: " + voId + "\n" +
            ...
            response.getWriter().println("<h2 style='color: green; text-align: center;'>Verification Officer Registered Successfully!</h2>");
    ...
    private String generateVoId() {
        Random rnd = new Random();
        int num = 100000 + rnd.nextInt(900000); // Generates a 6-digit number
        return "VO" + num; // Example format: VO123456
    }
