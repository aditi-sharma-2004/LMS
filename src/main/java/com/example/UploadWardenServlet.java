...
@WebServlet("/UploadWardenServlet")
@MultipartConfig(maxFileSize = 16177215) // 16MB max file size
public class UploadWardenServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String hostelId = request.getParameter("hostel_id"); // Fetch selected hostel_id
        ...
        // Generate unique warden_id
        String wardenId = generateWardenId();

        try {
            Class.forName("com.mysql.cj.jdbc.Driver");
            conn = DBConnection.getConnection();

            String sql = "INSERT INTO wardens (warden_id, name, email, phone, Image, hostel_id) VALUES (?, ?, ?, ?, ?, ?)";
            stmt = conn.prepareStatement(sql);
            stmt.setString(1, wardenId);
            ...
            stmt.setString(6, hostelId);

            int row = stmt.executeUpdate();
               
            if (row == 0) {
                response.getWriter().println("Error: Could not insert warden record.");
                return;
            }
            ...
            // Store warden login credentials
            String insertLoginSQL = "INSERT INTO wlogin (warden_id, password) VALUES (?, ?)";
            int row2;
            try (PreparedStatement pstmt2 = conn.prepareStatement(insertLoginSQL)) {
                pstmt2.setString(1, wardenId);
            ...
            String subject = "Your Warden Account Details";
            ...
            response.getWriter().println("<h2 style='color: green; text-align: center;'>Warden Registered Successfully!</h2>");
    ...
    private String generateWardenId() {
        Random rnd = new Random();
        int num = 100000 + rnd.nextInt(900000); // Generates a 6-digit number
        return "W" + num; // Example format: W123456
    }
